...
const vcard = 'BEGIN:VCARD\n' //jangan ganti Sayang
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6285284051385:+6285284051385\n'  //ganti nomor kamu sayang
...
namabot = 'K4YOU BOT'
cr = '*「 R1DH0 」*' //ganti nama bot
faketoko = "K4YOU BOT" //ganti nama bot
lolkey = 'e3d6805ea05cefd5b8fc3a9c' //apiknya ya sayang
zeksapi = 'Ardy-ApiKey' //free apikey
/******** OWNER NUMBER**********/

const ownerNumber = ["6285284051385@s.whatsapp.net"]   //ganti nomor kamu sayang
...
client.version = [2, 2119, 6]
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('YT Fernazerツ','red'),color('X','lime'),color('LolHuman','red'))
})
...
case 'addbadword':
		...
		case 'help': 
		case 'menu':
            //Saya Mohon Jangan hapus tqto ya sayang
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
...
╰────────────────⊱
`
client.sendMessage(from, wew, image, {quoted: fharga, caption: zain, pushname, prefix, getLevelingXp, getLevelingLevel, sender, role })
break
		...
		case 'lpr':
					...
					client.sendMessage('6287863200063@s.whatsapp.net', options, text, {quoted: mek})
		...
		case 'request':
					...
					client.sendMessage('6287863200063@s.whatsapp.net', options, text, {quoted: fharga})
					client.sendMessage('6287863200063@s.whatsapp.net', options, text, {quoted: fharga})
					client.sendMessage('6287863200063@s.whatsapp.net', options, text, {quoted: fharga})
...
//MAKERMENU
        ...
        if (!isRegistered) return reply(ind.noregis())
                    ...
                    query = `${body.slice(5)}`
                    reply(ind.wait())
                    anu = await getBuffer(`https://pecundang.herokuapp.com/api/texttopng?teks=${query}`)
         ...
         if (!isRegistered) return reply(ind.noregis())
                    ...
                    anu = (buffer.result)
                    client.sendMessage(from, anu, image, { quoted: mek, caption:'Nih Tuanku\nJan Lupa Subrek Yt Ardy Store', contextInfo: { forwardingScore: 1000, isForwarded: true}})
	                await limitAdd(sender)
	                break
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    client.sendMessage(from, ini_buffer, image, { quoted: fharga, caption:'Nih Tuanku\nJan Lupa Subrek Yt Ardy Store', contextInfo: { forwardingScore: 1000, isForwarded: true}})
                    break
...
//BATES NGAB STALKING MENU
                   case 'igstalk':
		            ...
		            case 'githubstalk':
					if (!isRegistered) return reply(ind.noregis())
					get_result = await fetchJson(`https://lolhuman.herokuapp.com/api/github/${body.slice(13)}?apikey=KatoNiBoss`, {method: 'get'})
					get_result = get_result.result
